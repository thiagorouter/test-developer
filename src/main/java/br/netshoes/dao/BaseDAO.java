...
public class BaseDAO<T extends BaseEntity, I> implements Dao<T, I> {
    ...
    public long count() {
        ...
        criteriaQuery.select(builder.count(criteriaQuery.from(this.entityClass)));
    ...
    @Transactional(readOnly = true)
    public List<T> findAll(Integer maxResult) {
        final CriteriaBuilder builder = this.getEntityManager().getCriteriaBuilder();
        final CriteriaQuery<T> criteriaQuery = builder.createQuery(this.entityClass);
        criteriaQuery.from(this.entityClass);
